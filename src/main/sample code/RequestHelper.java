...
//		response.setContentType("text/html");
		...
		List<Employee> emps = eserv.getEmployees();
	 ...
	 */
	public static void processLogin(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException, UserNotFoundException, PasswordInvalidException {
		...
		Employee employee = eserv.confirmLogin(username, password);
		...
		if (employee.getId() > 0) {
			...
			// add the user to the session
			session.setAttribute("the-user", employee);
			...
			out.println("<h1>Welcome " + employee.getFirstName() + "!</h1>");
